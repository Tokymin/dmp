...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="J:/Projects/stable-diffusion-v1-4",
    ...
    parser.add_argument(
        ...
        default="RGB",
        choices=["RGB", "F"],
        help="Set `F` for depth, otherwise `RGB` (default)"
    )
    parser.add_argument(
        "--target_scale",
        type=float,
    ...
    parser.add_argument(
        "--self_attn_only",
        action="store_true",
        help="Only add lora to self-attention"
    )
    parser.add_argument(
        "--disable_prompts",
        action="store_true",
...
def main():
    ...
    progress_bar = tqdm(range(global_step, args.max_train_steps), ncols=100, disable=not accelerator.is_local_main_process)
    ...
    if accelerator.is_main_process:
        ...
        pipeline.unet.save_attn_procs(args.output_dir)
